...
class Shopware_Controllers_Frontend_PaymentLiteCoin extends Shopware_Controllers_Frontend_Payment implements \Shopware\Components\CSRFWhitelistAware
{
    /**
    ...
    /**
     * Whitelist notify- and webhook-action for litecoin
     */
    ...
    {
        if ($this->getPaymentShortName() == 'zwillaweb_payment_litecoin') {
    ...
    public function gatewayAction()
    {
        ...
        if ($user !== null && $amountdue > 0.15) {
            ...
            if (!in_array($currency_iso_code, $limited_currencies)) {
                ...
                /** https://cex.io/api/last_price/LTC/EUR **/
                //$value_in_LTC = file_get_contents("https://blockchain.info/toltc?currency=".$currency_iso_code."&value=".$amountdue."");
                $json = file_get_contents('https://cex.io/api/last_price/LTC/EUR');
                $json = json_decode($json, true);

                $value_in_LTC = 1/($json['lprice'] / $amountdue);

                if ($value_in_LTC > 0) {
                    /** @var TYPE_NAME $zw_extended_public_key */
                    $zw_extended_public_key = Shopware()->Config()->getByNamespace('ZwWebPaymentLiteCoin', 'zw_extended_public_key');
                    $zw_blockchain_api_key = Shopware()->Config()->getByNamespace('ZwWebPaymentLiteCoin', 'zw_blockchain_api_key');
                    $zw_callback_secret = Shopware()->Config()->getByNamespace('ZwWebPaymentLiteCoin', 'zw_callback_secret');

                    $config_secret = trim($zw_callback_secret);
                    $secret = strtoupper(md5($config_secret.'-'.$this->session->sUserId));

                    $url = Shopware()->Router()->assemble(array('action' => 'notify', 'controller' => 'PaymentLiteCoin', 'module' => 'frontend', 'secret' => $secret, 'forceSecure' => true));
                    ...
                    if ($address) {
                        ...
                        Shopware()->Db()->exec("INSERT INTO `zwilla_free_litecoin_address` 
                                (`id_order`,`value_in_LTC`,`address`,`status`,`crdate`) 
                            VALUES
                                ('".(int)$orderId."', '".(double)$value_in_LTC."', '".$address."', 'Pending', CURRENT_TIMESTAMP)");

                        $order->setComment('Please pay ' . $value_in_LTC . ' LTC to this address ' . $address . ' for order number ' . $orderNumber);

                        Shopware()->Models()->flush($order);
                        $this->View()->receivedAddress = 'YES';
                        $this->View()->litecoinAddress = $address;
                        $this->View()->valueInLTC = $value_in_LTC;
    ...
    public function notifyAction()
    {
        ...
        $value_in_CoinCent = $this->Request()->getParam('value');
        $secret = $this->Request()->getParam('secret');
        $config_secret = Shopware()->Config()->getByNamespace('ZwWebPaymentLiteCoin', 'zw_callback_secret');

        $result = Shopware()->Db()->fetchRow("SELECT * FROM `zwilla_free_litecoin_address` WHERE `address` = '".$address."'");
        if ($result) {
            $id_order = $result['id_order'];
            $to_pay_in_LTC = $result['value_in_LTC'];
            ...
            if ($secret_sent === $secret) {
                if ($confirmations <= 5) {
                    Shopware()->Db()->exec("INSERT IGNORE INTO `zwilla_free_litecoin_transaction` 
                        (`transaction_hash`,`address`,`confirmations`,`value_in_CoinCent`,`crdate`) 
                    VALUES
                        ('".$transaction_hash."', '".$address."', '".(int)$confirmations."', '".(double)$value_in_CoinCent."', CURRENT_TIMESTAMP)");

                    Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_address` SET `status` = 'AwaitingConfirmations' WHERE `address` = '".$address."'");
                    Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_transaction` SET `confirmations` = '".(int)$confirmations."' WHERE `transaction_hash` = '".$transaction_hash."'");
                    ...
                    Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_transaction` SET `confirmations` = '".(int)$confirmations."' WHERE `transaction_hash` = '".$transaction_hash."'");
                    $total_paid_in_CoinCent = (double)Shopware()->Db()->fetchOne("
                        SELECT SUM(`value_in_CoinCent`)
                        FROM `zwilla_free_litecoin_transaction`
                        WHERE `address` = '".$address."' AND `confirmations` > 5");

                    $total_paid_in_ltc = $total_paid_in_CoinCent / 100000000;
                    $order = Shopware()->Modules()->Order();

                    if ($total_paid_in_ltc >= $to_pay_in_LTC) {
                        if ($total_paid_in_ltc == $to_pay_in_LTC) {
                            $order->setPaymentStatus($id_order, 12, true, 'Paid');
                            $order->setOrderStatus($id_order, 1, false, 'In Process');
                            Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_address` SET `status` = 'Paid' WHERE `address` = '".$address."'");
                        } elseif ($total_paid_in_ltc > $to_pay_in_LTC) {
                            $order->setPaymentStatus($id_order, 12, true, 'OverPaid');
                            $order->setOrderStatus($id_order, 8, false, 'Clarification Required, OverPaid');
                            Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_address` SET `status` = 'OverPaid' WHERE `address` = '".$address."'");
                        }
                    } elseif ($total_paid_in_ltc < $to_pay_in_LTC) {
                        $order->setPaymentStatus($id_order, 11, true, 'UnderPaid');
                        Shopware()->Db()->exec("UPDATE `zwilla_free_litecoin_address` SET `status` = 'UnderPaid' WHERE `address` = '".$address."'");
                    }
