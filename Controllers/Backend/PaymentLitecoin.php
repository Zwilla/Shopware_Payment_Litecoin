...
class Shopware_Controllers_Backend_PaymentLiteCoin extends Shopware_Controllers_Backend_ExtJs
{
    /**
    ...
    public function getListAction()
    {
        ...
        $select = $this->get('db')
            ...
            ->from(array('o' => 's_order'), array(
                ...
                'shopId' => 'subshopID',
                'litecoin_address' => 'transactionID',
                ...
                'invoiceHash' => new Zend_Db_Expr('(' . $this->get('db')
                        ...
                        ->order('ID DESC')
                        ->limit(1) . ')'),
                'total_paid_in_CoinCent' => new Zend_Db_Expr('(' . $this->get('db')
                        ->select()
                        ->from(array('zwilla_free_litecoin_transaction'), array('SUM(value_in_CoinCent)'))
            ...
            ->joinLeft(
                array('ltc_array' => 'zwilla_free_litecoin_address'),
                'ltc_array.address = o.transactionID',
                array(
                    'ltcStatus' => 'ltc_array.status', 'valueInLTC' => 'ltc_array.value_in_LTC'
                )
            )
            ->joinLeft(
            ...
            ->where('p.name LIKE ?', 'zwillaweb_payment_litecoin')
            ->where('o.status >= 0')
        ...
        foreach ($rows as &$row) {
            ...
            $row['total_paid_in_ltc'] = $row['total_paid_in_CoinCent'] / 100000000;
        }
    ...
    /**
     * Will register the correct shop for a given litecoin_address.
     *
     * @param $litecoin_address
     */
    public function registerShopBylitecoin_address($litecoin_address)
    {
        ...
        ';
        $result = $this->get('db')->fetchOne($sql, array($litecoin_address));
    ...
    public function getDetailsAction()
    {
        ...
        if (isset($filter[0]['property']) && $filter[0]['property'] == 'litecoin_address') {
            $this->Request()->setParam('litecoin_address', $filter[0]['value']);
        }
        $litecoin_address = $this->Request()->getParam('litecoin_address');

        // Load the correct shop in order to use the correct api credentials
        $this->registerShopBylitecoin_address($litecoin_address);

        $select = $this->get('db')
            ->select()
            ->from(array('zwilla_free_litecoin_transaction' => 'zwilla_free_litecoin_transaction'), array(
                'transaction_hash' => 'transaction_hash',
                'confirmations' => 'confirmations',
                'value_in_CoinCent' => 'value_in_CoinCent',
                'crdate' => 'crdate',
                'update' => 'update'
            ))
            ->where('address = ?', $litecoin_address);
        ...
        foreach ($rows as &$row) {
            $nr++;
            $row['number'] = $nr;
            $row['value_in_ltc'] = $row['value_in_CoinCent'] / 100000000;
        }
    ...
    {
        $this->View()->loadTemplate("backend/payment_litecoin/app.js");
